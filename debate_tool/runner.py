...
async def _do_compact(log: "Log", cfg: dict, system_text: str) -> "tuple[str, int]":
    ...
    delta_entries = log.entries_since_seq(prev_compact_seq)
    ...
    *,
    ...
    for questioner in debaters:
        ...
        sys_prompt = (
            ...
            f"- questions 长度为 2 到 3\n"
            f"- 每个问题必须指向对方本轮发言中的具体说法\n"
            f"- 不要输出综合方案、实施路线图、结论性长文。"
        )
        user_ctx = json.dumps(input_payload, ensure_ascii=False, indent=2)
        ...
        challenged_name = _extract_cross_exam_target(
            result,
