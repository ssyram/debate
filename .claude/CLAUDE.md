...
- `debate_tool/runner.py` — Debate engine. Usage: `debate-tool run <topic.md> [--rounds N] [--dry-run]`
- `debate_tool/wizard.py` — TUI wizard (14-step curses state machine)
- `debate_tool/core.py` — Defaults, constants, YAML generation
...
3. `debate-tool build` launches wizard (web default, `--cli` for TUI)
4. Output: `{stem}_debate_log.md` + `{stem}_debate_summary.md`
5. API priority: per-debater config > topic-level config > env vars (`DEBATE_BASE_URL`, `DEBATE_API_KEY`)
